...
echo '<h3>'.$mod['name'].' '.$mod['version'].' <small>| '.$mod['description'].'</small></h3>';
echo '</div>';
...
/* if both types deactivated, activate system entries */
if($_SESSION['setType_system'] == '' && $_SESSION['setType_module'] == '') {
    $_SESSION['setType_system'] = 'system';
    $btn_class_system = 'active';
}
...
if(is_numeric($_REQUEST['delete'])){
	...
	if($cnt_changes > 0) {
		echo '<div class="alert alert-success">The entry has been deleted</div>';
	}
} /* eo delete */
...
if(isset($_POST['save_entry'])) {
	...
	if($cnt_changes == TRUE){
		$sys_message = "{OKAY} The entry has been saved";
	} else {
		$sys_message = "{error} The entry was not saved";
		echo '<hr><pre>';
		print_r($dbh->errorInfo());
		echo '</pre><hr>';
	}
...
if($_GET['set_shorthand'] != '') {
	...
	if(isset($_GET['l'])) {
		$l = basename($_GET['l']);
		if(is_file('../core/lang/'.$l.'/dict-frontend.php')) {
			ob_start();
			include '../core/lang/'.$l.'/dict-frontend.php';
...
$tplform = file_get_contents(SE_CONTENT."/modules/alp.mod/templates/acp_form.tpl");
$tplform = str_replace('{form_action}', "acp.php?tn=addons&sub=alp.mod&a=start", $tplform);
...
if($cnt_entries < 1) {
	echo '<p class="alert alert-info">No entries available.</p>';
}
...
echo '<a class="btn btn-default '.$btn_class_system.'" href="?tn=addons&sub=alp.mod&a=start&setType=System">System</a>';
echo '<a class="btn btn-default '.$btn_class_module.'" href="?tn=addons&sub=alp.mod&a=start&setType=Module">alp.mod</a>';
echo '</div>';


echo '<div class="btn-group ms-1">';
foreach($active_lang as $l) {
    echo '<a class="btn btn-default btn-sm '.$btn_class_system.'" href="?tn=addons&sub=alp.mod&a=start&setLang='.$l['sign'].'">';
    echo '<img src="'.$l['flag'].'" width="16">';
    echo '</a>';
}
echo '</div>';
...
echo '<form class="form-inline float-end" action="?tn=addons&sub=alp.mod&a=start" method="POST">';
echo '<div class="input-group">';
echo '<input type="text" class="form-control" name="alp_filter" value="'.$alp_filter.'"> ';
echo '<input type="submit" name="set_alp_filter" value="Filter" class="btn btn-default text-success">';
echo '<input type="submit" name="reset_alp_filter" value="Reset" class="btn btn-default">';
echo '</div>';
...
$tplfile = file_get_contents(SE_CONTENT."/modules/alp.mod/templates/acp_list.tpl");
...
for($i=0;$i<$cnt_entries;$i++) {
	...
	$entry_flag = '<img src="../core/lang/'.$entry_lang.'/flag.png" width="22">';
	$entry_origin = $show_entries[$i]['alp_origin'];
	$tr_class = ' ';
	
	$link_edit = '<a class="btn btn-default btn-sm" href="?tn=addons&sub=alp.mod&a=start&edit_id='.$entry_id.'">'.$lang['edit'].'</a>';
	$link_delete = '<a class="btn btn-default text-danger btn-sm" href="?tn=addons&sub=alp.mod&a=start&delete='.$entry_id.'" onclick="return confirm(\''.$lang['confirm_delete_data'].'\')">'.$lang['delete'].'</a>';
	
	if($entry_origin == 'system') {
		$link_edit = '<a class="btn btn-default btn-sm" href="?tn=addons&sub=alp.mod&a=start&set_shorthand='.$entry_shorthand.'&l='.$entry_lang.'">'.$lang['duplicate'].'</a>';
		$link_delete = '';
		$tr_class .= 'system-row ';
